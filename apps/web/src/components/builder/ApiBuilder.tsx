...
export default function ApiBuilder({ onComplete, initialData, prefill }: ExtendedProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const r = await fetch(`http://localhost:3001/api/api/${targetId}`); if (r.ok) { const d = await r.json(); setForm({ displayName: d.displayName||targetId, id: d.id||targetId, description: d.description||'', entrypoint: d.entrypoint||'./src/routes', baseUrl: d.baseUrl||'', tags: (d.tags||[]).join(', ') }); setIsEditMode(true) }
      } catch {}
    }; load()
  ...
  return (
    <div className="text-white">
      ...
      <button onClick={() => onComplete({ id: form.id, type: 'api', displayName: form.displayName, version: '1.0.0', status: 'draft', description: form.description, runtime: 'node', entrypoint: form.entrypoint, baseUrl: form.baseUrl, pure: false, tags: form.tags.split(',').map(s=>s.trim()).filter(Boolean), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() })} className="w-full py-3 bg-[#f5c518] text-black font-black text-[12px] tracking-[0.3em] hover:bg-white transition-colors">VYGENEROVAT MANIFEST</button>
    </div>
  )
}
