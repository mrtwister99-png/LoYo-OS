...
export default function Notes() {
  const NOTES_DIR = 'D:/dev/loyo-os/data/ja/poznamky'
  ...
  const handleNew = async () => {
    ...
    const fileName = `${nextNum}poznamka_${slugify(title)}.md`
          ...
          <div className="col-span-4 space-y-4">
            ...
            <div className="bg-white rounded-[16px] border border-black/[0.06] overflow-hidden">
              ...
              <div className="max-h-[60vh] overflow-y-auto divide-y divide-black/[0.04]">
                {filtered.length === 0 ? <div className="p-8 text-center text-[12px] text-black/30 mono">Žádné poznámky</div> : filtered.map((note: Note) => {
                  const isActive = note.file_name === selectedId
                  return <div key={note.file_name} onClick={() => setSelectedId(note.file_name)} className={`p-5 cursor-pointer transition hover:bg-[#F8F6F1] ${isActive ? 'bg-black text-white hover:bg-black' : 'bg-white'}`}><div className="font-bold text-[14px] leading-tight line-clamp-1">{note.title}</div><div className={`text-[12px] mt-2 line-clamp-2 leading-relaxed ${isActive ? 'text-white/60' : 'text-black/50'}`}>{note.content.replace(/^#.*\n\n?/, '').slice(0, 120)}</div><div className={`mono text-[10px] mt-3 ${isActive ? 'text-white/30' : 'text-black/30'}`}>{note.created_at} • {note.file_name}</div></div>
                })}
              </div>
            </div>
          </div>
          <div className="col-span-8">
            {selected ? <div className="bg-white rounded-[20px] border border-black/[0.06] overflow-hidden flex flex-col min-h-[70vh] shadow-[0_20px_60px_rgba(0,0,0,0.04)]"><div className="p-8 border-b border-black/[0.06]"><input value={editingTitle} onChange={e => setEditingTitle(e.target.value)} placeholder="Název poznámky" className="w-full text-4xl font-black tracking-tighter bg-transparent outline-none placeholder:opacity-20" /><div className="mt-3 flex gap-2 mono text-[11px] text-black/40"><span className="px-2.5 py-1 bg-[#F8F6F1] rounded-full border border-black/[0.06] truncate max-w-[340px]">{selected.file_path}</span><span className="px-2.5 py-1 bg-black text-white rounded-full">{selected.created_at}</span></div></div><textarea value={editingContent} onChange={e => setEditingContent(e.target.value)} placeholder="Obsah... markdown" className="flex-1 w-full p-8 text-[14px] leading-[1.7] bg-transparent outline-none resize-none min-h-[420px] font-mono placeholder:opacity-30" /><div className="p-5 border-t border-black/[0.06] flex justify-between items-center bg-[#F8F6F1]"><button onClick={handleDelete} className="text-[11px] px-4 py-2 rounded-full border border-[#FF3B30]/30 text-[#FF3B30] hover:bg-[#FF3B30] hover:text-white transition font-bold tracking-widest">SMAZAT</button><div className="flex gap-3 items-center"><span className="mono text-[10px] opacity-40">{saving ? 'Ukládám...' : isTauri ? 'Uloží se na disk • Rust FS' : 'localStorage'}</span><button onClick={handleSave} disabled={saving} className="px-8 py-3 bg-black text-white text-[11px] font-black tracking-[0.2em] rounded-[12px] hover:bg-zinc-900 disabled:opacity-30 transition">{saving ? '...' : 'ULOŽIT'}</button></div></div></div> : <div className="bg-white rounded-[20px] border border-dashed border-black/10 min-h-[70vh] flex items-center justify-center"><div className="text-center"><div className="text-[48px] font-black text-black/10">∅</div><div className="mt-4 text-[14px] text-black/30 mono">{notes.length === 0 ? 'Žádné poznámky • klikni + NOVÁ' : 'Vyber poznámku vlevo'}</div></div></div>}
          </div>
        </div>
      </div>
    </div>
