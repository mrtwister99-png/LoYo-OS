// D:\dev\loyo-os\apps\web\src\pages\Tasks.tsx
...
export default function Tasks() {
  ...
  const TASKS_BASE = 'D:/dev/loyo-os/data/ja/ukoly'
  const ACTIVE_DIR = `${TASKS_BASE}/aktivni`
  const DONE_DIR = `${TASKS_BASE}/hotove`

  const buildContent = (title: string, done: boolean, body: string) =>
    `# ${title || 'Bez názvu'}\n\n**Hotovo:** ${done}\n\n${body}`
  ...
  const loadTasks = async () => {
    try {
      ...
      const [active, done] = await Promise.all([
        invoke<Task[]>('list_tasks', { dir: ACTIVE_DIR }),
        invoke<Task[]>('list_tasks', { dir: DONE_DIR }),
      ])
      const list = [...(active || []), ...(done || [])]
      if (list.length > 0) {
        setTasks(list)
        if (!selectedId || !list.find(t => t.file_name === selectedId)) {
          setSelectedId(list[0].file_name)
        }
  ...
  useEffect(() => {
    ...
    pollingRef.current = setInterval(async () => {
      try {
        ...
        const list = [...(active || []), ...(done || [])]
        if (list) {
          const oldIds = tasks.map(t => t.file_name).sort().join('|')
          const newIds = list.map(t => t.file_name).sort().join('|')
          const selOnDisk = selectedId ? list.find(t => t.file_name === selectedId) : null
          const selInMem = tasks.find(t => t.file_name === selectedId)
  ...
  const toggleChecklistItem = async (lineIdx: number) => {
    ...
    try {
      await persist({ ...selected, title: titleClean }, newContent)
    } catch (e) {
      setStatus(`Chyba: ${e}`)
    }
  }
  ...
  // Uloží obsah do správné složky (aktivni/hotove) podle stavu "done" a smaže starý soubor při přesunu
  const persistMoved = async (task: Task, newContent: string, newDone: boolean) => {
    ...
    if (isTauri) {
      ...
      if (newPath !== oldPath) {
        await invoke('delete_task', { path: oldPath })
      }
      await loadTasks()
    } else {
  ...
  const toggleDoneQuick = async (task: Task) => {
    ...
    try {
      await persistMoved(task, newContent, newDone)
      setStatus(`✓ ${newDone ? 'Splněno' : 'Vráceno do otevřených'}: ${task.title}`)
    } catch (e) {
      setStatus(`Chyba: ${e}`)
    }
  }
  ...
  const handleToggleDone = async () => {
    ...
    try {
      await persistMoved({ ...selected, title: titleClean }, newContent, newDone)
      setStatus(`✓ ${newDone ? 'Splněno' : 'Vráceno do otevřených'}: ${titleClean}`)
    } catch (e) {
      setStatus(`Chyba: ${e}`)
    }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      await persist({ ...selected, title: titleClean }, newContent)
      setStatus(`✓ Uloženo: ${selected.file_name}`)
    } catch (e) {
      setStatus(`Chyba: ${e}`)
    }
    setSaving(false)
  ...
  const handleNew = async () => {
    ...
    const fileName = `${nextNum}ukol_${slugify(title)}.md`
    ...
    try {
      if (isTauri) {
        ...
        setSelectedId(fileName)
        localStorage.setItem('loyo-tasks', JSON.stringify([newTask, ...tasks]))
      }
    } catch (e) {
      setStatus(`Chyba: ${e}`)
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      if (isTauri) {
        ...
        setTasks(remain)
        setSelectedId(remain.length > 0 ? remain[0].file_name : '')
        localStorage.setItem('loyo-tasks', JSON.stringify(remain))
      }
      ...
      <div className="max-w-[1400px] mx-auto space-y-6">
        <div className="flex justify-between items-end">
          <div>
            <div className="mono text-[10px] tracking-[0.3em] text-black/40">
              LOYO OS • TASKS • {openCount} OTEVŘENÝCH • {isTauri ? 'TAURI FS' : 'LOCAL MOCK'}
            </div>
            ...
            <div className="text-sm text-black/50 mt-2 flex flex-wrap gap-2 items-center">
              ...
              <span className="font-mono text-[11px] bg-white border border-black/10 px-2.5 py-1 rounded-full">{TASKS_BASE}/aktivni,hotove</span>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white rounded-[16px] p-6 border border-black/[0.06]">
            <div className="mono text-[10px] tracking-widest text-black/30">OTEVŘENÉ</div>
            <div className="text-4xl font-black mt-3">{openCount}</div>
            <div className="text-[11px] text-black/40 mt-1">čekají na vyřízení</div>
          </div>
          <div className="bg-black text-white rounded-[16px] p-6">
            <div className="mono text-[10px] tracking-widest text-white/30">HOTOVÉ</div>
            <div className="text-4xl font-black mt-3">{doneCount}</div>
            <div className="text-[11px] text-white/50 mt-1">splněných úkolů celkem</div>
          </div>
          <div className="bg-white rounded-[16px] p-6 border border-black/[0.06]">
            <div className="mono text-[10px] tracking-widest text-black/30">AKTIVNÍ ÚKOL</div>
        ...
        <div className="grid grid-cols-12 gap-6">
          <div className="col-span-4 space-y-4">
            <div className="bg-white rounded-[16px] border border-black/[0.06] p-4 space-y-3">
              ...
              <div className="flex gap-2">
                {(['open', 'done', 'all'] as const).map(f => (
                  <button key={f} onClick={() => setFilterDone(f)} className={`flex-1 text-[10px] font-black tracking-widest py-2 rounded-[8px] transition ${filterDone === f ? 'bg-black text-white' : 'bg-[#F8F6F1] text-black/40 hover:text-black/70'}`}>
                    {f === 'open' ? 'OTEVŘENÉ' : f === 'done' ? 'HOTOVÉ' : 'VŠE'}
                  </button>
                ))}
              </div>
            </div>
            <div className="bg-white rounded-[16px] border border-black/[0.06] overflow-hidden">
              ...
              <div className="max-h-[60vh] overflow-y-auto divide-y divide-black/[0.04]">
                {filtered.length === 0 ? (
                  <div className="p-8 text-center text-[12px] text-black/30 mono">Žádné úkoly</div>
                ) : filtered.map(task => {
                  const isActive = task.file_name === selectedId
                  return (
                    <div key={task.file_name} onClick={() => setSelectedId(task.file_name)} className={`p-5 cursor-pointer transition hover:bg-[#F8F6F1] flex gap-3 items-start ${isActive ? 'bg-black text-white hover:bg-black' : 'bg-white'}`}>
                      <input
                        type="checkbox"
                        checked={task.done}
                        onClick={e => e.stopPropagation()}
                        onChange={() => toggleDoneQuick(task)}
                        className="mt-1 w-4 h-4 accent-[#00D084] cursor-pointer flex-shrink-0"
                      />
            ...
            {selected ? (
                ...
                <div className="p-8 border-b border-black/[0.06]">
                  <div className="flex items-center gap-4">
                    <input
                      ...
                      className="w-6 h-6 accent-[#00D084] cursor-pointer flex-shrink-0"
                    />
                ...
                {checklistItems.length > 0 && (
                  <div className="px-8 pt-6 flex flex-col gap-2 border-b border-black/[0.06] pb-6">
                    ...
                    {checklistItems.map(item => (
                      <label key={item.lineIdx} className="flex items-center gap-3 text-[14px] cursor-pointer">
                        <input
                          type="checkbox"
                          checked={item.checked}
                          onChange={() => toggleChecklistItem(item.lineIdx)}
                          className="w-5 h-5 accent-[#00D084] cursor-pointer flex-shrink-0"
                        />
              ...
              <div className="bg-white rounded-[20px] border border-dashed border-black/10 min-h-[70vh] flex items-center justify-center">
                <div className="text-center">
                  <div className="text-[48px] font-black text-black/10">∅</div>
                  <div className="mt-4 text-[14px] text-black/30 mono">{tasks.length === 0 ? 'Žádné úkoly • klikni + NOVÝ ÚKOL' : 'Vyber úkol vlevo'}</div>
                </div>
              </div>
            )}
          </div>
